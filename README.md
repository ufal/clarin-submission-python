...
dc.description[required=true type=textarea], <b>"Description Title\n\nDescription text, or whatever else"</b>
...
local.submission.note[type=textarea],
...
Note, here, only the following metadata values are defined: **dc.type, dc.title, dc.date.issued, dc.contributor.author, local.contact.person, local.sponsor, dc.description, metashare.ResourceInfo#ContentInfo.detailedType** or **license.granted** and **clarin-license.name** where
...
- **dc.date.issued** (single required) date value is defined by user
...
- **dc.description** (single required) text value is defined by user. Note that, for the **textarea** type new lines (\n) can be included to separate lines.
